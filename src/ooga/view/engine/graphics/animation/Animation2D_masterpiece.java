...
/**
 * an class that implements 2D animation and extends from the Animation class
 * In addition to the basic mechanism provided in its parent class,
 * it implemented the specific nuances to 2D animation such as to load, update or return the animation texture at the current frame.
 * @author qiaoyi fang
 */
public class Animation2D_masterpiece extends Animation{
   ...
   * @param dir directory
   */
  public Animation2D_masterpiece(int cnt, int fps, String dir) {
    super(cnt,fps);
   ...
   */
  public Animation2D_masterpiece(int cnt, int fps, String dir, String action) {
    super(cnt,fps);
    this.animatedFrames = new Material[cnt];
   ...
   * @param fps frame per second
   */
  public Animation2D_masterpiece(int cnt, int fps) {
    super(cnt, fps);
    this.animatedFrames = new Material[cnt];
  }
   ...
   */
  public static Animation2D_masterpiece combineAnimations(Animation2D_masterpiece animation_1, Animation2D_masterpiece animation_2){

    int totalFrames = animation_1.getFrameAmount()+animation_2.getFrameAmount();
    Animation2D_masterpiece combined = new Animation2D_masterpiece(totalFrames, DEFAULT_FPS_2D);
